...
import { extractToken, verifyToken, hashPw } from '../../backend/utils/auth';
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      try {
        ...
        // MongoDB E11000 = duplicate key (email already exists)
    ...
    if (req.method === 'PUT') {
      ...
      if (tempPassword && String(tempPassword).length >= 8) {
        ...
        // Shorter than 8 chars — reject with helpful message
